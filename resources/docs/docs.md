...
The `home-routes` handler in the `efp-clojure-text-sharing.routes.home` namespace
...
The `home-page` function will in turn call the `efp-clojure-text-sharing.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `efp-clojure-text-sharing.handler` namespace:
...
Request middleware functions are located under the `efp-clojure-text-sharing.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `efp-clojure-text-sharing.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `efp-clojure-text-sharing.core` in some other namespace.
